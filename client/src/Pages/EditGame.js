...
const EditGame = ({ gameId, onBack }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const finalData = {
            ...
            keywords: typeof formData.keywords === 'string' ? formData.keywords.split(',').map(i => i.trim()) : formData.keywords, 
            screenshots: typeof formData.screenshots === 'string' ? formData.screenshots.split(',').map(i => i.trim()) : formData.screenshots,
    ...
    return (
        <div className="admin-container">
            <div className="admin-header">
                <h1>Edit Game: {formData.title}</h1>
            ...
            <form onSubmit={handleSubmit} className="admin-form">
                <div className="form-section">
                    ...
                    <div className="form-row">
                        <div className="form-group"><input type="text" name="id" placeholder="Unique ID (e.g. gta-v)" value={formData.id} onChange={handleChange} required className="form-input" disabled /></div>
                ...
                <button type="submit" className="submit-btn" style={{width: '100%', padding: '15px'}}>Update Game Info</button>
            </form>
        </div>
    );
};
